...
`;customElements.define("better-switch-card-editor",class extends st{static get properties(){return{hass:{type:Object},config:{type:Object}}}setConfig(t){this.config=t}get _entity(){return this.config.entity||""}get _name(){return this.config.name||""}get _icon(){return this.config.icon||""}get _animation_duration(){return this.config.animation_duration||500}_valueChanged(t){if(!this.config||!this.hass)return;const e=t.target;if(!e.configValue)return;let i=t.detail?.value||e.value;"animation_duration"===e.configValue&&(i=parseInt(i)||500);const s={...this.config,[e.configValue]:i},n=new CustomEvent("config-changed",{detail:{config:s},bubbles:!0,composed:!0});this.dispatchEvent(n)}render(){return this.hass&&this.config?j`
